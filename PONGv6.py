...
def cycle_menu_color():
    ...
    if game_state == "MENU":
        ...
        menu_title_turtle.write("PONG v6", align="center", font=("Arial", 60, "bold"))

        # Increment the index, wrap around to 0 -T
        COLOR_INDEX = (COLOR_INDEX + 1) % len(MENU_COLORS)
        
        # Schedule next color change (2000 milliseconds = 2 seconds, dont give players a seizure via setting to 2) -T
...
def game_loop():
    ...
    if game_state == "MENU":
        ...
        hit_ball.hideturtle()
        obstacle.hideturtle() #Removes annoying pong net so gameover text is actually visible -T
